...
// Sending Email to applicant
 ...
 $('#emailModal').on('show.bs.modal', function (event) {
   debugger;
    ...
    $(".email-form [name=job_id]").val(button.data('id'));
    var modal = $(this)
    modal.find('.modal-title').text('New message to ' + recipient)
    modal.find('.modal-body #email').val(recipient);
    modal.find('.modal-body #message').val(feedBack);
    ...
    send_email.click(function(e) {
      ...
      $.ajax({
          data: _form,
          type: 'POST',
          url: '<?php echo base_url();?>employers/applicants/feedback',
          success: function(response){
            debugger;
            $(this).removeData('bs.modal');
            $.notify("Feedback has been sent Successfully", "success");
            $('.close').trigger('click');
          }
      });
      e.preventDefault();
